...
let lineWraps = (self, width, start, last) => {
  ...
  let rec calcLines = (lines) => {
    if (curStart^ >= last || curStart^ >= textLength) {
      ...
      while (curStart^ < nextNewline && String.get(self.text, curStart^) == ' ') {
        curStart := curStart^ + 1;
      };
      ...
      if (lineEnd^ < nextNewline) {
        ...
        while (lineEnd^ > curStart^ && String.get(self.text, lineEnd^) != ' ') {
          lineEnd := lineEnd^ - 1;
        };
        if (lineEnd^ == curStart^) {
          ...
          while (lineEnd^ > curStart^ && String.get(self.text, lineEnd^ - 1) == ' ') {
            lineEnd := lineEnd^ - 1;
          };
        };
      };
...
let update = (self : textLayout) => {
  ...
  let maxLineWidth = List.fold_left((max, fitting) => {
    (fitting.width > max) ? fitting.width : max
  }, 0, lines);
  let penPos = {
    x: 0,
    y: 0
  ...
  let (_, glyphs) = List.fold_left(((lineIndex, glyphs), line) => {
    ...
    let rec forChar = (charIndex, glyphs) => {
      if (charIndex >= line.last) {
        ...
        let id = Char.code(String.get(self.text, charIndex));
        let bmChar = switch(self.font.charsById[id]) {
        | Some(bmChar) => bmChar
        | None => failwith("Could not find char");
        };
        switch (lastGlyph^) {
        | Some(lastGlyph : BMFont.bmChar) =>
          penPos.x = penPos.x + BMFont.getKerning(self.font, lastGlyph.id, bmChar.id);
        | None => ()
        };
        let tx = switch (self.align) {

...
let fromBase64 = (base64) => {
    ...
    let rec loop = (i, j) => {
        let tmp = (
            (((revLookup[base64[i]] lsl 18) lor
            (revLookup[base64[i + 1]] lsl 12)) lor
...
let readUInt32LE = (bytes, offset) => {
    bytes[offset] lor
    (bytes[offset + 1] lsl 8) lor
    (bytes[offset + 2] lsl 16) +
...
let readInt32LE = (bytes, offset) => {
    bytes[offset] lor
    (bytes[offset + 1] lsl 8) lor
    (bytes[offset + 2] lsl 16) lor
    bytes[offset + 3] lsl 24
};
...
let utf8Slice = (bytes, start, last) => {
    let rec getCodePoints = (i, codePoints) => {
        if (i >= last) {
            ...
            let codePoint = ref(-1);
            let bytesPerSequence = if (firstByte > 0xEF) {
                4
            ...
            if (i + bytesPerSequence <= last) {
                ...
                | 2 =>
                    ...
                    if ((secondByte land 0xC0) == 0x80) {
                        let tempCodePoint = (firstByte land 0x1F) lsl 0x6 lor (secondByte land 0x3F);
                ...
                | 3 =>
                    ...
                    if ((secondByte land 0xC0) == 0x80 && (thirdByte land 0xC0) == 0x80) {
                        let tempCodePoint = (firstByte land 0xF) lsl 0xC lor (secondByte land 0x3F) lsl 0x6 lor (thirdByte land 0x3F);
                ...
                | 4 =>
                    ...
                    if ((secondByte land 0xC0) == 0x80 && (thirdByte land 0xC) == 0x80 && (fourthByte land 0xC0) == 0x80) {
                        let tempCodePoint = (
                            (firstByte land 0xF) lsl 0x12 lor
                            (secondByte land 0x3F) lsl 0xC lor
                            (thirdByte land 0x3F) lsl 0x6 lor
                            (fourthByte land 0x3F)
                        );
